...
void MainWindow::doubleClickImageList()
{
  ...
  size_t id;
  is >> id;
  if (m_doc.m_image_list.size()>id)
  {

    const std::string sView = m_dImageFile+"/"+m_doc.m_image_list[id];
    m_widget->AddImage(QString::fromStdString(sView), 0.f, 0.f, true);
    m_widget->SetCurrentViewId(id);
    ...
    for (Landmarks::iterator iterL = m_doc.m_control_points.begin();
      iterL != this->m_doc.m_control_points.end(); ++iterL)
    {
      ...
      if (obs.count(id) != 0)
      {
        ...
        m_widget->AddNode(new control_point_2DNode(QPointF(x,y), x, y, index));
      }
    }
...
{
  const QString dir = QString::fromStdString("/home/wuwei/");
  const QString gcp_data_filename = QFileDialog::getSaveFileName(this, tr("Choose a gcp file"),
    dir, tr("gcp_file (*.txt *.gcp)"));

  std::setlocale(LC_ALL, "C");
  if (!m_doc.saveData(gcp_data_filename.toStdString()))
  {
    QMessageBox msgBox;
    msgBox.setText("Cannot save the gcp file.");
    msgBox.exec();
  }
}
...
void MainWindow::openProject()
{
  ...
  const QString dir=fileDialog->getExistingDirectory(this,tr("Choose a image directory"));
  if (dir.isEmpty())
    return;

  m_dImageFile = dir.toStdString();

  if (m_doc.loadData(dir.toStdString()))
  {
    //Add image names in the QT tree view
    {
      ...
      view_ids.reserve(m_doc.m_image_list.size());
      for(int i=0;i<m_doc.m_image_list.size();++i){
        view_ids.push_back(i);
      }

      // Add view in reverse order to have them ordered by ID
      for (std::vector<int>::iterator iter = view_ids.begin();
        iter != view_ids.end(); ++iter)
      {
          std::ostringstream os;
          os << *iter << " " << m_doc.m_image_list[*iter];
          model->insertRow(0);
...
void MainWindow::editControlPoints()
{
  ...
  if (!m_doc.m_control_points.empty())
  {
    using namespace control_point_GUI;
    ControlPointTableView control_point_editor(m_doc.m_control_points, this);
    control_point_editor.exec();
    Landmarks control_points_cpy = m_doc.m_control_points;
    control_point_editor.update_control_points(control_points_cpy);
    m_doc.m_control_points = control_points_cpy;
  }
  else
  {
    QMessageBox msgBox;
...
void MainWindow::registerProject()
{
  ...
  for (Landmarks::const_iterator iterL = m_doc.m_control_points.begin();
    iterL != m_doc.m_control_points.end(); ++iterL)
  {
