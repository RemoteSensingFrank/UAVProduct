...
: QDialog(parent)
{
  ...
  const int row_count = control_points.size();
  ...
  int i = 0;
  for (Landmarks::const_iterator iterL = control_points_.begin();
    iterL != control_points_.end(); ++iterL, ++i)
  {
    // ID, #obs, X, Y, Z
    ...
    // X
    item = new QTableWidgetItem( QString::number(iterL->second.X) );
    table_->setItem(i,2,item);
    // Y
    item = new QTableWidgetItem( QString::number(iterL->second.Y) );
    table_->setItem(i,3,item);
    // Z
    item = new QTableWidgetItem( QString::number(iterL->second.Z) );
...
)
{
  ...
  for (int i = 0; i < table_->rowCount(); ++i)
  {
    ...
    {
      control_points_[index].X = valueX;
      control_points_[index].Y = valueY;
      control_points_[index].Z = valueZ;
    }
    else
    {
      QMessageBox msgBox;
