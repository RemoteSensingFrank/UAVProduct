...
namespace control_point_GUI
{
  ...
  GraphicsMainWindow::GraphicsMainWindow(Document & doc, QWidget * parent)
    ...
    view(new QGraphicsView(scene)), _doc(doc), _current_view_id(-1)
  {
  ...
  void GraphicsMainWindow::mousePressEvent (QMouseEvent* e )
  {
    ...
    if (index != -1)
    {
      ...
      Landmark & landmark = _doc.m_control_points[index];
      if (landmark.obs.count(_current_view_id) == 0)
      {
        landmark.obs[_current_view_id].x = pos.x();
        landmark.obs[_current_view_id].y = pos.y();
        landmark.obs[_current_view_id].id_feat = 0;
        // Create a graphical instance that points directly to the control point observation
        double x = landmark.obs[_current_view_id].x;
        double y = landmark.obs[_current_view_id].y;
        this->AddNode(new control_point_2DNode(pos, x, y, index));
      }
      else
      {
        QMessageBox msgBox;
