...
UAVErr UAVProcessBundle::UAVProcessBundleGlobal(std::shared_ptr<UAVProcessFeature> feats_ptr,std::string mathces,std::string sfm_in,std::string sfm_out) 
{
    ...
    sfmEngine.SetRotationAveragingMethod(
        openMVG::sfm::ERotationAveragingMethod(int (openMVG::sfm::ROTATION_AVERAGING_L2)));
    sfmEngine.SetTranslationAveragingMethod(
        openMVG::sfm::ETranslationAveragingMethod(int (openMVG::sfm::TRANSLATION_AVERAGING_SOFTL1)));
...
UAVErr UAVProcessBundle::UAVProcessBundleSquence(std::shared_ptr<UAVProcessFeature> feats_ptr,std::string mathces,std::string sfm_in,std::string sfm_out)
{
    ...
    if (sfmEngine.Process())
    {
        std::cout << std::endl << " Total Incremental-Sfm took (s): " << timer.elapsed() << std::endl;
...
{
    openMVG::sfm::Landmarks &sfmGcps = sfm_data.control_points;
    int existedLandMarks = sfmGcps.size();
    if(existedLandMarks==0)
        sfmGcps = m_gcps;
    else{
        for(auto iter : m_gcps){
            sfmGcps.at(existedLandMarks) = iter.second;
            ++existedLandMarks;
        }
    }
}

UAVErr UAVProcessBundleGCPs::UAVProcessBundleGlobal(std::shared_ptr<UAVProcessFeature> feats_ptr, std::string mathces,
                                                    std::string sfm_in, std::string sfm_dout) {
    if(feats_ptr== nullptr)
        return 7;
    unique_ptr<openMVG::sfm::Features_Provider> feat_provider = feats_ptr->UAVProcessFeatsProvide();
    if(feat_provider== nullptr)
    ...
    openMVG::sfm::GlobalSfMReconstructionEngine_RelativeMotions sfmEngine(
            sfm_data,
            stlplus::folder_part(sfm_dout),
            stlplus::create_filespec(stlplus::folder_part(sfm_dout), "Reconstruction_Report.html"));
    ...
    if (sfmEngine.Process())
    {
        ...
        std::cout << "...Generating SfM_Report.html" << std::endl;
        Generate_SfM_Report(sfmEngine.Get_SfM_Data(),
                            stlplus::create_filespec(sfm_dout, "SfMReconstruction_Report.html"));
        ...
        Save(sfmEngine.Get_SfM_Data(),
             stlplus::create_filespec(stlplus::folder_part(sfm_dout), "cloud_and_poses", ".ply"),
             openMVG::sfm::ESfM_Data(openMVG::sfm::ALL));
...
UAVErr UAVProcessBundleGCPs::UAVProcessBundleSquence(std::shared_ptr<UAVProcessFeature> feats_ptr,std::string mathces,
                                                     std::string sfm_in,std::string sfm_dout)
{
    if(feats_ptr== nullptr)
        return 7;
