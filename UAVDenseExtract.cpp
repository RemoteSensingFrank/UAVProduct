...
{
    //读取ply数据
    MVS::PointCloud pntcl;

    if(!pntcl.Load(pathPly)) {
    ...
    for(int i=0;i<pntcl.GetSize();++i){
        //坐标是不是要转换到UTM坐标系下
        Vec3 lla;
        lla=ecef_to_lla(pntcl.points[i].x,pntcl.points[i].y,pntcl.points[i].z);
        Vec3 utm;
        ...
        dfXMax = (utm(0)>dfXMax ) ? utm(0) : dfXMax;
        dfYMin = (utm(1)<dfYMin ) ? utm(1) : dfYMin;
        dfYMax = (utm(1)>dfYMax ) ? utm(1) : dfYMax;
        dfZMin = (utm(2)<dfZMin ) ? utm(2) : dfZMin;
        dfZMax = (utm(2)>dfZMax ) ? utm(2) : dfZMax;
    }
   ...
   // poOptions->dfRadius1 = 3;
    ...
    GDALGridCreate(GGA_InverseDistanceToAPower, poOptions, pntcl.GetSize(), points3d[0], points3d[1], points3d[2],
    ...
    GDALDataset *poDataset = pDriver->Create(pathDsm.c_str(), nXSize,nYSize, 1, GDT_Float32, NULL);

    // 设置六参数
    double adfGeoTransform[6] = {dfXMin, dGround, 0 , dfYMax, 0, -dGround};
